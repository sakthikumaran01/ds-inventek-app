...
describe("structuredData", () => {
  it("getOrganizationJsonLd produces valid JSON with an Organization and a LocalBusiness per address", () => {
    ...
    expect(org.alternateName).toBe("DS Robotics");
    expect(org.email).toBe(content.company.email);
    // Only the confirmed LinkedIn/YouTube URLs are asserted as sameAs —
    // instagram/whatsapp in content.json are still unverified guesses.
    expect(org.sameAs).toEqual([
      content.company.socialLinks.linkedin,
      content.company.socialLinks.youtube,
    ]);
